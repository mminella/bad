...
template <class T>
tdiff_t
meth1_merge_copy( T * s1, T * e1, T * s2, T * e2, T * rs, uint64_t cap )
{
  auto t0 = time_now();
...
template <typename T>
tdiff_t
meth1_pmerge_copy( T * s1, T * e1, T * s2, T * e2, T * rs, uint64_t cap )
{
  auto t0 = time_now();
