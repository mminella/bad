...
pair<string, uint16_t> Address::ip_port( void ) const
{
  ...
  const int gni_ret =
    getnameinfo( &to_sockaddr(), size_, ip, sizeof( ip ), port, sizeof( port ),
...
const in_addr & Address::to_inaddr( void ) const
{
  return ( (struct sockaddr_in *)&to_sockaddr() )->sin_addr;
}

const in6_addr & Address::to_in6addr( void ) const
{
  return ( (struct sockaddr_in6 *)&to_sockaddr() )->sin6_addr;
}
